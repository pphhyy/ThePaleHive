...
{
    public class JobGiver_InsectoidSow : ThinkNode
    {
        public override ThinkResult TryIssueJobPackage(Pawn pawn, JobIssueParams jobParams)
        {
            WorkGiver_GrowerSow workgiver = PPhhyyWorkGiverDefOfs.GrowerSow.Worker as WorkGiver_GrowerSow;
            ...
            else
            {
                foreach (IntVec3 cell in workgiver.PotentialWorkCellsGlobal(pawn))
                {
                    ...
                    if (!cell.IsForbidden(pawn) && workgiver.HasJobOnCell(pawn, cell))
                    {
                ...
                Job sowJob;
                sowJob = workgiver.JobOnCell(pawn, targetInfo.Cell, false);
                if (sowJob != null)
                {
                    return new ThinkResult(sowJob, this, new JobTag?(workgiver.def.tagToGive), false);
                }
            }
            return ThinkResult.NoJob;
        }
